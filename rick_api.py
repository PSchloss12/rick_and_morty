...
def gallery():
    ...
    return render_template(
        "gallery.html", first_col=first_col, second_col=second_col, third_col=third_col
    )
...
def location(loc_id: int):
    ...
    for i in chars:  # fetch characters at location from db
        try:
            loc.characters.append(
                db_session.execute(select(Character).filter_by(id=i)).first()[0]
            )
